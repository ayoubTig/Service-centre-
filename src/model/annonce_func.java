...
DateFormat formatDat = new SimpleDateFormat("yyyy/MM/dd");
			...
			String sql = "INSERT INTO `annonce`(`ID_off`, `image`, `descreption`,`etat`,`date`,`price`,`title`,`ID_categorie`) VALUES (?,?,?,?,?,?,?,?);";
			...
			preparedStatement.setString(4, a.getEtat());
			preparedStatement.setObject(5, localDate);
			preparedStatement.setInt(6,a.getPrice());
	...
	public static int update(int id, String etat) throws SQLException
	{
		int check = 0;
		...
		try{
			...
			String sql = "UPDATE `annonce` SET  `etat`=?   WHERE ID_annonce=?";
			Connection con = (Connection) connect.loadDatabase();
			PreparedStatement preparedStatement = (PreparedStatement) con.prepareStatement(sql);
	...
	public static Annonce getAnnonce(int id) throws SQLException
	{
		...
		try{DateFormat formatDate = new SimpleDateFormat("yyyy-MM-dd");
			...
			String sql = "SELECT * FROM `annonce` WHERE `ID_annonce`='"+id+"';";
			Connection con = (Connection) connect.loadDatabase();
			PreparedStatement preparedStatement = (PreparedStatement) con.prepareStatement(sql);
	...
	public static List<Annonce> get_Annonce_By_Categorie(String cate) throws SQLException
	{
		...
		try{
			...
			String sql = "SELECT `ID_annonce`, `ID_off`, `image`, `descreption`, `date`, `etat`, `price`, `title`, annonce.`ID_categorie`,`Type` FROM `annonce`,`categorie` where annonce.`ID_categorie` =categorie.`ID_categorie` and nom_categorie='"+cate+"'";
			Connection con = (Connection) connect.loadDatabase();
			Statement statement = (Statement) con.createStatement();
	...
	public static ArrayList<Annonce> accepteAnnonce() throws SQLException
	{
		...
		try{
			...
			while(res.next())
			{
				...
				ann.setDescription(res.getString(4));
				ann.setId_cat(res.getInt(9));
				ann.setDatepub(res.getString(5)); 
				ann.setEtat(res.getString(6));	
				ann.setTitle(res.getString(8));
				ann.setPrice(res.getInt(7));
				annonces.add(ann);
...
System.out.println("etape2");
	...
	public static  double  sum_des_Star(int id_off) throws SQLException
	{
		...
		String sq="SELECT sum(`nbr_star`)  from  `evaluation`  WHERE `ID_off`= "+id_off ;
		double  cate=0.0;
